...
def create_table():
    ...
    with connect_to_db() as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute("DROP TABLE IF EXISTS qa_rag_ontology;")
            # Creazione tabella (se non esiste)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS qa_rag_ontology (
...
def process_questions(questions, graph):
    ...
    insert_query = """
    INSERT INTO qa_rag_ontology (id, question_text, ground_truth, answer_orca, response_time_orca)
    VALUES (%s, %s, %s, %s, %s)
    """
...
with connect_to_db() as conn:
    query = "SELECT * FROM qa_rag_ontology;"
    df = pd.read_sql_query(query, conn)
    csv_path = os.path.join(dataset_path, "qa_results.csv")
    df.to_csv(csv_path, index=False)

mlflow.log_artifact(csv_path)
print(f"File CSV `qa_rag_ontology.csv` salvato come artefatto su MLflow.")
...
with connect_to_db() as conn:
    with conn.cursor() as cursor:
        ...
        cursor.execute("SELECT id, ground_truth, answer_orca FROM qa_rag_ontology;")
